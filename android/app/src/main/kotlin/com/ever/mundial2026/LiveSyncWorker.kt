...
class LiveSyncWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    private fun processLiveNotifications(context: Context, status: MatchStatus, teams: TeamLookup) {
        ...
        if (status.isLive && total > 0 && (previousTotal == -1 || total > previousTotal)) {
            NotificationHelper.notifyOnce(
                ...
                "${strings(context).goalIn} ${matchName(status, teams)}",
                scoreBody(status, teams),
            )
        }
    ...
    private fun checkKickoffReminders(
        ...
        for (match in matches) {
            ...
            NotificationHelper.notifyOnce(
                ...
                "${matchName(match, teams)} ${strings(context).starts(text)}",
            )
        }
    }

    private fun matchName(status: MatchStatus, teams: TeamLookup): String {
        return "${teamName(status.match.homeSlot, status.homeEspn, teams)} vs ${teamName(status.match.awaySlot, status.awayEspn, teams)}"
    }

    private fun matchName(match: MatchInfo, teams: TeamLookup): String {
        return "${teamName(match.homeSlot, "", teams)} vs ${teamName(match.awaySlot, "", teams)}"
    }

    private fun scoreBody(status: MatchStatus, teams: TeamLookup): String {
        val base = if (status.homeScore == null || status.awayScore == null) {
            matchName(status, teams)
        } else {
            "${teamName(status.match.homeSlot, status.homeEspn, teams)} ${status.homeScore} - ${status.awayScore} ${teamName(status.match.awaySlot, status.awayEspn, teams)}"
        }
        return if (status.detail.isBlank()) base else "$base · ${status.detail}"
    }

    private fun teamName(slot: String, espnName: String, teams: TeamLookup): String {
        val language = NotificationHelper.language(applicationContext)
    ...
    private fun strings(context: Context): WorkerStrings {
...
private data class WorkerStrings(
    val goalIn: String,
    val fullTime: String,
